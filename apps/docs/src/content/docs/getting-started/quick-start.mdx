...
import QuickStartDemoWrapper from '../../../components/demos/QuickStartDemoWrapper.astro';
...
```tsx
import {createImageId} from 'osdlabel/core';
import type {ImageSource} from 'osdlabel/core';
...
The `Annotator` component renders:

- **Toolbar** — Tool selector that respects context constraints, showing which tools are available and their usage counts
- **Grid View** — One or more image viewers in a configurable grid (use `=`/`-` keys to resize)
- **Filmstrip** — Sidebar for assigning images to grid cells (click an image to assign it to the active cell)
