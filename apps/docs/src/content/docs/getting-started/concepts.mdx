...
osdlabel is composed of three layers:

1. **OpenSeaDragon** — Manages the DZI/tiled image viewer, handling pan, zoom, and tile loading
2. **Fabric.js overlay** — A transparent Fabric.js canvas positioned on top of each OSD viewer, synchronized on every animation frame
3. **SolidJS state** — Reactive stores that drive the UI, tool selection, constraints, and annotation data
...
```ts
import {createImageId, createAnnotationId, createAnnotationContextId} from 'osdlabel/core';
...
Each grid cell contains an OpenSeaDragon viewer with a `FabricOverlay` on top. The overlay operates in one of two modes, routing input based on the active state:

- **Navigation mode** (`'navigation'`) — This is the default mode when no tool is selected. The `FabricOverlay`'s mouse tracker is disabled, passing all pointer events directly to OpenSeaDragon. Fabric.js objects are rendered but cannot be interacted with.
- **Annotation mode** (`'annotation'`) — This mode is activated when a tool is selected (including the `select` tool). The overlay intercepts all pointer events, forwarding them as synthetic events to Fabric.js. OSD's native navigation is disabled. To pan or zoom in this mode, users can bypass Fabric by pressing `Ctrl`/`Cmd` while dragging or scrolling.

In a multi-cell grid, the currently active grid cell operates in its intended mode (often annotation), while all other background cells operate strictly in navigation mode.
