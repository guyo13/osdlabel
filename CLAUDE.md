...
- **`@osdlabel/annotation`** (`packages/annotation/`) — Pure annotation data model. Zero dependencies. Types are split into dedicated modules: `annotation.ts` (`BaseAnnotation`, `Annotation<E>`, `AnnotationStyle`, `AnnotationId`), `annotation-tool.ts` (`ToolType`), `geometry.ts` (geometry discriminated unions), `types.ts` (`ImageId`, generic `RawAnnotationData<TFormat, TData>`, `ImageSource`), `util.ts` (`createAnnotationId`, `createImageId`, `toolTypeToGeometryType`), `id.ts`, `constants.ts`.
- **`@osdlabel/viewer-api`** (`packages/viewer-api/`) — Viewer state types and utilities. Contains: `AnnotationState<E>`, `getAllAnnotationsFlat`, `CellTransform`, `DEFAULT_CELL_TRANSFORM`, `UIState`, `KeyboardShortcutMap`. Depends on `@osdlabel/annotation`.
...
- **`@osdlabel/validation`** (`packages/validation/`) — Valibot schema implementations (Standard Schema compatible). Contains: `GeometrySchema`, `PointSchema`, `BaseAnnotationSchema`, `FabricRawAnnotationDataSchema`. Depends on `@osdlabel/annotation` and `valibot`.
...
- **`@osdlabel/validation` depends only on `@osdlabel/annotation` and `valibot`.** No framework deps. All validation is Valibot-based — there are no manual validators in `@osdlabel/annotation`.
...
2. **`@osdlabel/viewer-api`** — Viewer state types and utilities. `import { type AnnotationState, type UIState, type CellTransform, type KeyboardShortcutMap, DEFAULT_CELL_TRANSFORM, getAllAnnotationsFlat } from '@osdlabel/viewer-api'`
...
4. **`@osdlabel/validation`** — Validation schemas. `import { BaseAnnotationSchema, GeometrySchema, FabricRawAnnotationDataSchema } from '@osdlabel/validation'`
